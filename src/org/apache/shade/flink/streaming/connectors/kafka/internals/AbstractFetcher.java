...
package org.apache.shade.flink.streaming.connectors.kafka.internals;
...
import org.apache.shade.flink.streaming.connectors.kafka.internals.metrics.KafkaConsumerMetricConstants;
...
public abstract class AbstractFetcher<T, KPH> {
	...
	protected AbstractFetcher(
		...
		this.legacyCurrentOffsetsMetricGroup = consumerMetricGroup.addGroup(KafkaConsumerMetricConstants.LEGACY_CURRENT_OFFSETS_METRICS_GROUP);
		this.legacyCommittedOffsetsMetricGroup = consumerMetricGroup.addGroup(KafkaConsumerMetricConstants.LEGACY_COMMITTED_OFFSETS_METRICS_GROUP);
	...
	private void registerOffsetMetrics(
		...
		for (KafkaTopicPartitionState<KPH> ktp : partitionOffsetStates) {
			MetricGroup topicPartitionGroup = consumerMetricGroup
				.addGroup(KafkaConsumerMetricConstants.OFFSETS_BY_TOPIC_METRICS_GROUP, ktp.getTopic())
				.addGroup(KafkaConsumerMetricConstants.OFFSETS_BY_PARTITION_METRICS_GROUP, Integer.toString(ktp.getPartition()));

			topicPartitionGroup.gauge(KafkaConsumerMetricConstants.CURRENT_OFFSETS_METRICS_GAUGE, new OffsetGauge(ktp, OffsetGaugeType.CURRENT_OFFSET));
			topicPartitionGroup.gauge(KafkaConsumerMetricConstants.COMMITTED_OFFSETS_METRICS_GAUGE, new OffsetGauge(ktp, OffsetGaugeType.COMMITTED_OFFSET));
