...
import org.apache.shade.flink.streaming.connectors.kafka.config.StartupMode;
import org.apache.shade.flink.streaming.connectors.kafka.internals.KafkaTopicPartition;
import org.apache.shade.flink.streaming.connectors.kafka.partitioner.FlinkFixedPartitioner;
import org.apache.shade.flink.streaming.connectors.kafka.partitioner.FlinkKafkaPartitioner;
import org.apache.flink.table.api.TableException;
import org.apache.flink.table.api.TableSchema;
import org.apache.flink.table.descriptors.DescriptorProperties;
import org.apache.shade.flink.table.descriptors.KafkaValidator;
...
public abstract class KafkaTableSourceSinkFactoryBase implements
	...
	public Map<String, String> requiredContext() {
		...
		context.put(CONNECTOR_TYPE, KafkaValidator.CONNECTOR_TYPE_VALUE_KAFKA); // kafka
	...
	public List<String> supportedProperties() {
		...
		// update mode
//		properties.add(UPDATE_MODE); cfbber
		System.err.println("特性丢失--->UPDATE_MODE"  );

		// kafka
		properties.add(KafkaValidator.CONNECTOR_TOPIC);
		properties.add(KafkaValidator.CONNECTOR_PROPERTIES);
		properties.add(KafkaValidator.CONNECTOR_PROPERTIES + ".#." + KafkaValidator.CONNECTOR_PROPERTIES_KEY);
		properties.add(KafkaValidator.CONNECTOR_PROPERTIES + ".#." + KafkaValidator.CONNECTOR_PROPERTIES_VALUE);
		properties.add(KafkaValidator.CONNECTOR_PROPERTIES + ".*");
		properties.add(KafkaValidator.CONNECTOR_STARTUP_MODE);
		properties.add(KafkaValidator.CONNECTOR_SPECIFIC_OFFSETS);
		properties.add(KafkaValidator.CONNECTOR_SPECIFIC_OFFSETS + ".#." + KafkaValidator.CONNECTOR_SPECIFIC_OFFSETS_PARTITION);
		properties.add(KafkaValidator.CONNECTOR_SPECIFIC_OFFSETS + ".#." + KafkaValidator.CONNECTOR_SPECIFIC_OFFSETS_OFFSET);
		properties.add(KafkaValidator.CONNECTOR_SINK_PARTITIONER);
		properties.add(KafkaValidator.CONNECTOR_SINK_PARTITIONER_CLASS);
		...
		// computed column
//		properties.add(SCHEMA + ".#." + TABLE_SCHEMA_EXPR); cfbber
		System.err.println("特性丢失 ++++ table_schema-expr");
		...
		// format wildcard
//		properties.add(FORMAT + ".*"); cfbber
		System.err.println("特性丢失 ++++ formate");
	...
	public StreamTableSource<Row> createStreamTableSource(Map<String, String> properties) {
		...
		final String topic = descriptorProperties.getString(KafkaValidator.CONNECTOR_TOPIC);
	...
	private Properties getKafkaProperties(DescriptorProperties descriptorProperties) {
		...
		if (KafkaValidator.hasConciseKafkaProperties(descriptorProperties)) {
			descriptorProperties.asMap().keySet()
				.stream()
				.filter(key -> key.startsWith(KafkaValidator.CONNECTOR_PROPERTIES))
				.forEach(key -> {
					final String value = descriptorProperties.getString(key);
					final String subKey = key.substring((KafkaValidator.CONNECTOR_PROPERTIES + '.').length());
					kafkaProperties.put(subKey, value);
				});
		} else {
			final List<Map<String, String>> propsList = descriptorProperties.getFixedIndexedProperties(
				KafkaValidator.CONNECTOR_PROPERTIES,
				Arrays.asList(KafkaValidator.CONNECTOR_PROPERTIES_KEY, KafkaValidator.CONNECTOR_PROPERTIES_VALUE));
			propsList.forEach(kv -> kafkaProperties.put(
				descriptorProperties.getString(kv.get(KafkaValidator.CONNECTOR_PROPERTIES_KEY)),
				descriptorProperties.getString(kv.get(KafkaValidator.CONNECTOR_PROPERTIES_VALUE))
			));
		}
	...
	private StartupOptions getStartupOptions(
		...
		final StartupMode startupMode = descriptorProperties
			.getOptionalString(KafkaValidator.CONNECTOR_STARTUP_MODE)
			.map(modeString -> {
				switch (modeString) {
	...
	private void buildSpecificOffsets(DescriptorProperties descriptorProperties, String topic, Map<KafkaTopicPartition, Long> specificOffsets) {
		if (descriptorProperties.containsKey(KafkaValidator.CONNECTOR_SPECIFIC_OFFSETS)) {
			...
			final List<Map<String, String>> offsetList = descriptorProperties.getFixedIndexedProperties(
					KafkaValidator.CONNECTOR_SPECIFIC_OFFSETS,
					Arrays.asList(KafkaValidator.CONNECTOR_SPECIFIC_OFFSETS_PARTITION, KafkaValidator.CONNECTOR_SPECIFIC_OFFSETS_OFFSET));
			offsetList.forEach(kv -> {
				final int partition = descriptorProperties.getInt(kv.get(KafkaValidator.CONNECTOR_SPECIFIC_OFFSETS_PARTITION));
				final long offset = descriptorProperties.getLong(kv.get(KafkaValidator.CONNECTOR_SPECIFIC_OFFSETS_OFFSET));
				final KafkaTopicPartition topicPartition = new KafkaTopicPartition(topic, partition);
				specificOffsets.put(topicPartition, offset);
			});
		}
	...
	private Optional<FlinkKafkaPartitioner<Row>> getFlinkKafkaPartitioner(DescriptorProperties descriptorProperties) {
		return descriptorProperties
			.getOptionalString(KafkaValidator.CONNECTOR_SINK_PARTITIONER)
			.flatMap((String partitionerString) -> {
				switch (partitionerString) {
					case KafkaValidator.CONNECTOR_SINK_PARTITIONER_VALUE_FIXED:
						return Optional.of(new FlinkFixedPartitioner<>());
					case KafkaValidator.CONNECTOR_SINK_PARTITIONER_VALUE_ROUND_ROBIN:
						return Optional.empty();
					case KafkaValidator.CONNECTOR_SINK_PARTITIONER_VALUE_CUSTOM:
						final Class<? extends FlinkKafkaPartitioner> partitionerClass =
							descriptorProperties.getClass(KafkaValidator.CONNECTOR_SINK_PARTITIONER_CLASS, FlinkKafkaPartitioner.class);
