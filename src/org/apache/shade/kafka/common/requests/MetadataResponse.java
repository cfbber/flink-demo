...
public class MetadataResponse extends AbstractRequestResponse {
    ...
    public MetadataResponse(Cluster cluster, Map<String, Errors> errors) {
        ...
        List<org.apache.shade.kafka.common.protocol.types.Struct> brokerArray = new ArrayList<org.apache.shade.kafka.common.protocol.types.Struct>();
        for (Node node : cluster.nodes()) {
            org.apache.shade.kafka.common.protocol.types.Struct broker = struct.instance(BROKERS_KEY_NAME);
        ...
        for (Map.Entry<String, Errors> errorEntry : errors.entrySet()) {
            org.apache.shade.kafka.common.protocol.types.Struct topicData = struct.instance(TOPIC_METATDATA_KEY_NAME);
            topicData.set(TOPIC_KEY_NAME, errorEntry.getKey());
            topicData.set(TOPIC_ERROR_CODE_KEY_NAME, errorEntry.getValue().code());
            topicData.set(PARTITION_METADATA_KEY_NAME, new org.apache.shade.kafka.common.protocol.types.Struct[0]);
        ...
        for (String topic : cluster.topics()) {
            if (!errors.containsKey(topic)) {
                ...
                for (PartitionInfo fetchPartitionData : cluster.partitionsForTopic(topic)) {
                    org.apache.shade.kafka.common.protocol.types.Struct partitionData = topicData.instance(PARTITION_METADATA_KEY_NAME);
    ...
    public MetadataResponse(org.apache.shade.kafka.common.protocol.types.Struct struct) {
        super(struct);
        ...
        for (int i = 0; i < brokerStructs.length; i++) {
            org.apache.shade.kafka.common.protocol.types.Struct broker = (org.apache.shade.kafka.common.protocol.types.Struct) brokerStructs[i];
        ...
        for (int i = 0; i < topicInfos.length; i++) {
            org.apache.shade.kafka.common.protocol.types.Struct topicInfo = (org.apache.shade.kafka.common.protocol.types.Struct) topicInfos[i];
            ...
            if (topicError == Errors.NONE.code()) {
                ...
                for (int j = 0; j < partitionInfos.length; j++) {
                    org.apache.shade.kafka.common.protocol.types.Struct partitionInfo = (org.apache.shade.kafka.common.protocol.types.Struct) partitionInfos[j];
